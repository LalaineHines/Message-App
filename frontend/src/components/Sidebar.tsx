    ...
    <div className={`h-full p-5 rounded-r-xl overflow-y-auto text-white/80 ${selectedUser ? "max-lg:hidden" : ''}`}>
      <div className="pb-5">
        <div className="flex justify-between items-center pl-[1vh]">
            ...
            <div className="relative py-2" ref={menuRef}>
                ...
                {isMenuOpen && (
                <div className="absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-neutral-800 text-white/80 block">
      ...
      >
        ...
        <div className="flex flex-col leading-5">
          <p className="text-sm font-semibold text-sky-200 flex items-center gap-1">Whisp Bot <img src={assets.verified} alt="whisp bot" className="h-4 w-4" /></p>
          <span className="text-neutral-500 text-xs font-medium">AI Assistant</span>
        </div>
        <img src={assets.pin} alt="" className="h-4 w-4" />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {loading
          ? Array(5)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className="flex items-center gap-4 p-2 pl-4 rounded-xl animate-pulse"
                >
                  <div className="w-[40px] h-[40px] bg-neutral-600 rounded-full flex items-center justify-center text-lg">
                  </div>
        ...
        <p className="text-center text-xs text-neutral-600 mt-[5vh]">Explore the Friends tab to grow your network!</p>
      </div>
